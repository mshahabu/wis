...
/* TODO - Temporary to print all records */    
    ...
    public function enter_student_info($info_s) {
        ...
        list ( $month, $day, $year ) = split ( '[/.-]', $_REQUEST ['register_date'] );
    ...
    public function update_student_record() {
        ...
        if (! empty ( $_REQUEST ['register_date'] )) {
            $reg_rec ['register_date'] = convert_normal_date_to_SQL ( $_REQUEST ['register_date'] );
        }
        if (! empty ( $_REQUEST ['auth_person1'] )) {
        ...
        if (! empty ( $_REQUEST ['waiver_signed_by'] )) {
    ...
    private function modify_student_pers_info($readonly = TRUE) {
        ...
        print "<tr>";
        print "<td>Registration Date (mm/dd/yyyy) </em></td>";
        print "<td><input type=text name='register_date' value='" . $reg_date . "' " . $read_attr . " ></td>";
    ...
    public function studentReturnRegistration($sid, &$info=NULL)
    {
        ...
        print "<input type=hidden name='student_id' value='" . $sid . "'>";

        if($info == NULL)
        {
            $this->get_records($sid);
            $motherPhone = explode("-",$this->infoParent['mother_cell_phone']);
            $fatherPhone = explode("-",$this->infoParent['father_cell_phone']);
            $homePhone   = explode("-",$this->infoPersonalInfo['home_phone']);

            //Student 1 info
            $info['first_name1']        = $this->infoPersonalInfo['first_name'];
            $info['last_name1']         = $this->infoPersonalInfo['last_name'];
            $info['middle_name1']       = $this->infoPersonalInfo['middle_name'];
            $info['ps_grade1']          = $this->infoRegistration['reg_school_grade'];
            $info['email1']             = $this->infoPersonalInfo['email'];
            $info['allergies1']         = $this->infoRegistration['allergies'];
            $info['medications1']       = $this->infoRegistration['medications'];
            $info['register_date']      = $this->infoRegistration['register_date'];
            $info['waiver_signed_by']   = $this->infoRegistration['waiver_signed_by'];
            $info['form_signed_by']     = $this->infoRegistration['form_signed_by'];
            
            //Mother info
            $info['mother_first_name']  = $this->infoParent['mother_first_name'];
            $info['mother_middle_name'] = $this->infoParent['mother_middle_name'];
            $info['mother_last_name']   = $this->infoParent['mother_last_name'];
            $info['mother_cell_area']   = $motherPhone[0];
            $info['mother_cell_local']  = $motherPhone[1];
            $info['mother_cell_number'] = $motherPhone[2];

            //Father info
            $info['father_first_name']  = $this->infoParent['father_first_name'];
            $info['father_middle_name'] = $this->infoParent['father_middle_name'];
            $info['father_last_name']   = $this->infoParent['father_last_name'];
            $info['father_cell_area']   = $fatherPhone[0];
            $info['father_cell_local']  = $fatherPhone[1];
            $info['father_cell_number'] = $fatherPhone[2];
 
            $info['home_area']          = $homePhone[0];
            $info['home_local']         = $homePhone[1];
            $info['home_number']        = $homePhone[2];
            
            $info['par_email'] = $this->infoParent['par_email'];

            //Address
            $info['address'] = $this->infoPersonalInfo['address'];
            $info['city']    = $this->infoPersonalInfo['city'];
            $info['zipcode'] = $this->infoPersonalInfo['zipcode'];
            $info['state']   = $this->infoPersonalInfo['state'];

            $info['parent_volun_date1'] = convert_sql_date_to_normal($this->infoRegistration['parent_volun_1']);
            $info['parent_volun_date2'] = convert_sql_date_to_normal($this->infoRegistration['parent_volun_2']);

            //Authorized person info
            $info['auth_person1']       = $this->infoRegistration['auth_person1'];
            $info['phone_ap1']          = $this->infoRegistration['phone_ap1'];
            $info['driver_lic_ap1']     = $this->infoRegistration['driver_lic_ap1'];
            $info['address_ap1']        = $this->infoRegistration['address_ap1'];

            $info['auth_person2']       = $this->infoRegistration['auth_person2'];
            $info['phone_ap2']          = $this->infoRegistration['phone_ap2'];
            $info['driver_lic_ap2']     = $this->infoRegistration['driver_lic_ap2'];
            $info['address_ap2']        = $this->infoRegistration['address_ap2'];
        }
        ...
        $this->input_text_field ( 'register_date', 'readonly', $today, 10, 10 );
...
//        print '><label> ICSGV member </label>';
        ...
        print "<tr>";
        print "<td><B style='font-size:13px'> Child Info</B>";
        print "</td><td>";

        $this->input_text_field ( 'first_name1', 'readonly', $info['first_name1'], 10, 20, true );
        print "&nbsp";
        $mn = empty ( $info ['middle_name1'] ) ? '' : $info ['middle_name1'];
        $this->input_text_field ( 'middle_name1', 'readonly', $mn, 1, 2 );
        print "&nbsp";

        $this->input_text_field ( 'last_name1', 'readonly', $info['last_name1'], 10, 20, true );

        print "</td><td style='font-size:13px'>";
        $this->input_text_field ( 'email1', $readonly, $info['email1'], 30, 50 );

        print "</td>";
        print "<td>";

        $this->input_text_field ( 'ps_grade1', $readonly, $info['ps_grade1'], 4, 4, true );
        ...
        $this->input_text_field ( 'mother_first_name', 'readonly', $info['mother_first_name'], 10, 20, true );
        print "</td>";
        print "<td> ";

        $this->input_text_field ( 'mother_middle_name', 'readonly', $info['mother_middle_name'], 1, 2 );
        print "</td>";
        print "<td> ";

        $this->input_text_field ( 'mother_last_name', 'readonly', $info['mother_last_name'], 10, 20, true );
        print "</td>";
        ...
        $this->input_text_field ( 'mother_cell_area', $readonly, $info ['mother_cell_area'], 3, 3, true );
        print "&nbsp";

        $this->input_text_field ( 'mother_cell_local', $readonly, $info ['mother_cell_local'], 3, 3, true );
        print "-";

        $this->input_text_field ( 'mother_cell_number', $readonly, $info ['mother_cell_number'], 4, 4, true );
        print "</td>";
        print "</tr>";
        ...
        print "<td> ";
        $this->input_text_field ( 'father_first_name', 'readonly', $info['father_first_name'], 10, 20, true );
        print "</td>";

        print "<td> ";
        $this->input_text_field ( 'father_middle_name', 'readonly', $info['father_middle_name'], 1, 2 );
        print "</td>";

        print "<td> ";
        $this->input_text_field ( 'father_last_name', 'readonly', $info['father_last_name'], 10, 20, true );
        print "</td>";
        ...
        $this->input_text_field ( 'father_cell_area', $readonly, $info ['father_cell_area'], 3, 3 );
        print "&nbsp";

        $this->input_text_field ( 'father_cell_local', $readonly, $info ['father_cell_local'], 3, 3 );
        print "-";

        $this->input_text_field ( 'father_cell_number', $readonly, $info ['father_cell_number'], 4, 4 );
        print "</td>";
        ...
        $this->input_text_field ( 'address', $readonly, $info['address'], 22, 55, true );
        print "</td><td>";
        print "<label style='font-size:14px'>City</label>";
        print "<label style='color:red;font-size:11px'>*</label>";
        print "</td><td>";

        $this->input_text_field ( 'city', $readonly, $info['city'], 15, 30, true );
        print "</td><td>";
        print "<label style='font-size:14px' >Home</label>";
        print "<label  style='color:red;font-size:11px'>*</label>";
        print "</td><td>";

        $this->input_text_field ( 'home_area', $readonly, $info ['home_area'], 3, 3, true );

        print "&nbsp";
        $this->input_text_field ( 'home_local', $readonly, $info ['home_local'], 3, 3, true );
        print "-";

        $this->input_text_field ( 'home_number', $readonly, $info ['home_number'], 4, 4, true );
        print "<br>";
        print "</td></tr>";
        ...
        $this->input_text_field ( 'state', $readonly, 'California', 11, 15 );
        print "</td><td>";
        ...
        $this->input_text_field ( 'zipcode', $readonly, $info['zipcode'], 5, 5, true );
        print "</td><td>";
        ...
        $this->input_text_field ( 'par_email', $readonly, $info['par_email'], 30, 50, true );
        print "</td></tr>";
        print "</table>";
        // print "<br>";
        ...
        $this->input_text_field ( "parent_volun_date1", $readonly, $info ['parent_volun_date1'], 10, 10, false, 'datepicker' );

        print "<label  style='font-size:14px'>Date - 2 (mm/dd/yyyy)</label>";
        print "<em style='color:red;'> &nbsp</b></em>";

        $this->input_text_field ( "parent_volun_date2", $readonly, $info ['parent_volun_date2'], 10, 10, false, 'datepicker' );
        ...
        print '<label  style="font-size:14px">Your signature below will give permission for your child to participate in all school activities within the premises of Islamic Center of San Gabriel Valley (ICSGV) or outside activities held in conjunction with Weekend Islamic School. It will also waive all the claims against ICSGV for injury, accident, illness, or death during any school activities. </label>';
        print "<B style='color:red'> Your Name below constitute to legal signature </B>"; 
        print '<BR>';
        print '<BR>';
        print '<label style="font-size:14px"><I><B>Signature of parent, guardian or student 18 years of age or older</I></B></label>';
        print '<BR>';
        print "<input name='waiver_signed_by' placeholder='Father or mother exact name' ";
        if(!empty($info['waiver_signed_by'])){
            print " value='" . $info['waiver_signed_by'] ."'";
        }
        print " style='color:blue' size=45 maxlength=45 required />";
        ...
        print "<td>";
        print "</td><td>";
        $this->input_text_field ( "allergies1", $readonly, $info['allergies1'], 25, 40 );
        print "</td><td>";
        $this->input_text_field ( "medications1", $readonly, $info['medications1'], 25, 40 );
        print "</td></tr>";
        ...
        print '<label style="font-size:14px"><I><B>Signature of parent, guardian or student 18 years of age or older </B></I></label>';
        print '<BR>';
        print "<input name='form_signed_by' placeholder='Father or mother exact name' ";
        if(!empty($info['waiver_signed_by'])){
            print " value='" . $info['waiver_signed_by'] ."'";
        }
        print "style='color:blue' size=45 maxlength=45 required />";
        ...
        print "<td>";
        $this->input_text_field ( "auth_person1", $readonly, $info['auth_person1'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "address_ap1", $readonly, $info['address_ap1'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "phone_ap1", $readonly, $info['phone_ap1'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "driver_lic_ap1", $readonly, $info['driver_lic_ap1'], 15, 24 );
        print "</td>";
        print "</tr>";

        print "<tr>";
        print "<td>";
        $this->input_text_field ( "auth_person2", $readonly, $info ['auth_person2'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "address_ap2", $readonly, $info ['address_ap2'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "phone_ap2", $readonly, $info ['phone_ap2'], 15, 24 );
        print "</td>";

        print "<td>";
        $this->input_text_field ( "driver_lic_ap2", $readonly, $info ['driver_lic_ap2'], 15, 24 );
        print "</td>";
    ...
    public function studentReturnRegistrationDb($sid, &$info)
    {
        ...
        $piInfo['first_name']  = $info['first_name1'];
        $piInfo['middle_name'] = $info['middle_name1'];
        $piInfo['last_name']   = $info['last_name1'];
        $piInfo['cell_phone']  = "";
        $piInfo['home_phone']  = $info['home_area'] . '-' . $info['home_local'] . '-' . $info['home_number'];
        $piInfo['email']       = $info['email1'];
        $piInfo['address']     = $info['address'];
        $piInfo['city']        = $info['city'];
        $piInfo['zipcode']     = $info['zipcode'];
        $piInfo['state']       = $info['state'];

        $this->personalInfoIf->update_record($piInfo, $pid);


        $parentId = $this->studentIf->get_parent_id($sid);

        $parInfo['mother_last_name']   = $info['mother_last_name'];
        $parInfo['mother_first_name']  = $info['mother_first_name'];
        $parInfo['mother_middle_name'] = $info['mother_middle_name'];
        $parInfo['mother_cell_phone']  = $info['mother_cell_area'] . '-' . $info['mother_cell_local'] . '-' . $info['mother_cell_number'];

        $parInfo['father_last_name']   = $info['father_last_name'];
        $parInfo['father_first_name']  = $info['father_first_name'];
        $parInfo['father_middle_name'] = $info['father_middle_name'];
        $parInfo['father_cell_phone']  = $info['father_cell_area'] . '-' . $info['father_cell_local'] . '-' . $info['father_cell_number'];
        $parInfo['par_email']          = $info['par_email'];
        ...
        if (! empty ( $info ['parent_volun_date1'] )) {
            ...
            $regInfo['parent_volun_1'] = NULL;
        }
        if (! empty ( $info ['parent_volun_date2'] )) {
            $regInfo['parent_volun_2']    = convert_normal_date_to_SQL ( $info ['parent_volun_date2'] );
        } else {
            $regInfo['parent_volun_2'] = NULL;
        }
        $regInfo['allergies']         = $info['allergies1'];
        $regInfo['medications']       = $info['medications1'];
        $regInfo['register_date']     = convert_normal_date_to_SQL ( $info ['register_date'] );
        
        $regInfo['auth_person1']      = $info['auth_person1'];
        $regInfo['address_ap1']       = $info['address_ap1'];
        $regInfo['phone_ap1']         = $info['phone_ap1'];
        $regInfo['driver_lic_ap1']    = $info['driver_lic_ap1'];

        $regInfo['auth_person2']      = $info['auth_person2'];
        $regInfo['address_ap2']       = $info['address_ap2'];
        $regInfo['phone_ap2']         = $info['phone_ap2'];
        $regInfo['driver_lic_ap2']    = $info['driver_lic_ap2'];

        $regInfo['waiver_signed_by']  = $info['waiver_signed_by'];
        $regInfo['form_signed_by']    = $info['form_signed_by'];
    ...
    private function input_text_field($name, $readonly, $value, $size = NULL, $maxlen = NULL, $required = false, $class = NULL) {
    ...
    function eSignatureInput($name)
    {
        //<form action="submit.php" onsubmit="return onSubmit(this)" method="post">
        print "<div>";
        print "<input name=$name placeholder='Father or mother exact name' size=50 maxlength=70 required />";
        print "</div>";
        //print "<div>";
        //print "<canvas id='signature' width='300' height='100'></canvas>";
        //print "</div>";
        //print "<div>";
        //print "<input type='hidden' name='signature' />";
        //print "</div>";
